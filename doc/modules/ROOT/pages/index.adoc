...
to cluster members and clients using Kerberos tickets in the Microsoft Active Directory domain.
...
// Now we have to workaround the fact our client application is running under the
// same account as one of the members on the Windows Server. The default credential
// cache file used by Java (`C:\Users\hazelcast\krb5cc_hazelcast`) already contains member credentials,
...
You can also also reconfigure log level for `com.hazelcast.security` category to inspect
...
[source,properties]
----
include::../../../../debug-logging.properties[]
----
...
We have configured Hazelcast Enterprise cluster with Kerberos based authentication
on Active Directory domain.

First, we used the simple Kerberos configuration for just for member authentication.
Then we added full configuration and also included the client configuration.
